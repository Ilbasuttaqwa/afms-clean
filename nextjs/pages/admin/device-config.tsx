...
export default function DeviceConfigPage() {
  ...
  const fetchDevices = async () => {
    try {
      ...
      addToast(error.message || 'Gagal memuat data device', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveConfig = async () => {
    try {
      ...
      if (selectedDevice) {
        ...
        if (response.success) {
          addToast('Konfigurasi device berhasil diperbarui', 'success');
          fetchDevices();
        ...
        if (response.success) {
          addToast('Device baru berhasil ditambahkan', 'success');
          fetchDevices();
          setShowConfigModal(false);
        }
      }
    } catch (error: any) {
      addToast(error.message || 'Gagal menyimpan konfigurasi device', 'error');
    } finally {
  ...
  const handleDeleteDevice = async (device: Device) => {
    ...
    try {
      ...
      if (response.success) {
        addToast('Device berhasil dihapus', 'success');
        fetchDevices();
      }
    } catch (error: any) {
      addToast(error.message || 'Gagal menghapus device', 'error');
    }
  };
  ...
  const handleClearFingerprintData = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        addToast('Semua data fingerprint berhasil dihapus', 'success');
      ...
      addToast(error.message || 'Gagal menghapus data fingerprint', 'error');
    } finally {
